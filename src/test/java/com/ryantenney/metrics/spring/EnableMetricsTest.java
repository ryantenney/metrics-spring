...
public class EnableMetricsTest {
	...
	public void metricsConfigTest() throws Throwable {
		...
		assertThat(fieldGauge.value(), is(5));
		...
		assertThat(methodGauge.value(), is(6));
		...
		assertThat(timedMethodTimer.count(), is(0L));
		testBean.timedMethod();
		assertThat(timedMethodTimer.count(), is(1L));
		...
		assertThat(meteredMethodMeter.count(), is(0L));
		testBean.meteredMethod();
		assertThat(meteredMethodMeter.count(), is(1L));

		// Verify that the Meter's counter is incremented on method invocation
		Meter exceptionMeteredMethodMeter = forExceptionMeteredMethod(metricsRegistry, TestBean.class, "exceptionMeteredMethod");
		assertThat(exceptionMeteredMethodMeter.count(), is(0L));
		try {
			testBean.exceptionMeteredMethod();
		} catch (Throwable t) {}
		assertThat(exceptionMeteredMethodMeter.count(), is(1L));
	}
